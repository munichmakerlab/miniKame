...
int angToUsec(float value){
  ...
  return map((value/180 * (MAX_PULSE_WIDTH-MIN_PULSE_WIDTH) + MIN_PULSE_WIDTH), MIN_PULSE_WIDTH, MAX_PULSE_WIDTH, SERVOMIN, SERVOMAX);
}

void MiniKame::init(){
  // initial I2C pwm controller
  Wire.begin(D2, D1);    // ESP8266 can use any two pins, such as SDA to GPIO04/D2 and SCL to GPIO05/D1
  pwm.begin();
...
void MiniKame::setServo(int id, float target){
  ...
  pwm.setPWM(id, 0, angToUsec(target+trim[id]));
  yield();
